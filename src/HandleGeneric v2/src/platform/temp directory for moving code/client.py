...
class AzureOpenAIClient:
        ...
        **kwargs,
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                frequency_penalty=kwargs.get("frequency_penalty", self.config.AI_FREQUENCY_PENALTY),
                presence_penalty=kwargs.get("presence_penalty", self.config.AI_PRESENCE_PENALTY),
                **{
                    k: v
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            result = self.ask_question("Say hello and confirm you're working!", max_tokens=50)
            if result["status"] == "success":
                self.logger.info("Connection test successful")
                print(f"Connection successful! Response: {result['answer']}")
                return True
            else:
                self.logger.error(f"Connection test failed: {result.get('error', 'Unknown error')}")
...
def main():
    ...
    parser.add_argument("--max-tokens", "-m", type=int, help="Maximum tokens for response")
    parser.add_argument(

...
class CodeValidator:
    ...
    def _validate_single_file(self, file_path: str) -> ValidationResult:
        ...
        if file_path.endswith(".cs"):
            # cs-specific validation

            # Syntax check
            syntax_errors = self._check_cs_syntax(file_path)
    ...
    def _check_cs_syntax(self, file_path: str) -> List[str]:
        """Check C# file syntax using csc compiler"""
        errors = []

        try:
            # Run the C# compiler in syntax-check-only mode
            result = subprocess.run(
                ["csc", "/nologo", "/t:library", "/warn:4", "/nowarn:1591", file_path],
            ...
            error_msg = "C# compiler (csc.exe) not found. Ensure .NET SDK is installed and in PATH."
            errors.append(error_msg)
            logging.error(error_msg)

        except Exception as e:
            error_msg = f"Unexpected error while checking {file_path}: {str(e)}"
            errors.append(error_msg)
            logging.error(error_msg)
